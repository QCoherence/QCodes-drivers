...
class Vaunix_attn(Instrument):
    ...
    def _set_attn(self, attn):
        ...
        error = self._vnx.fnLDA_SetAttenuationHR(
            self._Devices[device_id], int(attn * 20)
        )
    ...
    def _set_working_freq(self, freq):
        device_id = self._find_serial()
        init_dev = self._vnx.fnLDA_InitDevice(self._Devices[device_id])
        error = self._vnx.fnLDA_SetWorkingFrequency(
            self._Devices[device_id], int(freq * 10000)
        )
    ...
    def get_idn(self):
        return {
            "vendor": "Vaunix digital attenuator",
            "model": "LDA-5018V",
            "serial": self._serial,
            "firmware": None,
        }
