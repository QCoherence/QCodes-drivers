...
class FixedFrequencyPointMagPhaseTrace(MultiParameter):
    """
    Custom class for 2 tones by Dorian.
    The difference with FixedFrequencyPointMagPhase is that it does not take
...
class FrequencySweepMagPhase(MultiParameter):
    ...
    def get_raw(self) -> Tuple[ParamRawDataType, ...]:
        ...
        return 20*np.log10(abs(data)), np.unwrap(np.angle(data))
...
class VNAChannel(InstrumentChannel):
    def __init__(
        self,
        parent: "VNA",
        name: str,
        ...
        self.add_parameter(
            name="point_fixed_frequency_mag_phase_trace",
            parameter_class=FixedFrequencyPointMagPhaseTrace,
        )
    ...
    def _set_trigger(self, val: str) -> None:
        channel = self._instrument_channel
        self.write(f"TRIG{channel}:SOUR {val}")

    def _set_trigger_link(self, val: str) -> None:
        channel = self._instrument_channel
...
class ZNB(VisaInstrument):
    """
    QCoDeS driver for the Rohde & Schwarz ZNB8 and ZNB20 and ZNL14
