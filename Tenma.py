...
class Tenma_72_52359(VisaInstrument):
    ...
    def __init__(self, name: str, address: str, channel = 1, **kwargs:Any) -> None:
        ...
        self.add_parameter(name='use_settings',
                           label='Settings',
                           get_cmd='RCL?',
                           set_cmd='RCL{}',)
        # Allows you to save settings.
        # Parameter ranges from 1 to 5.
        self.add_parameter(name='save_settings',
        ...
        # Current range : 0-3A
        # Voltage range : 0-30V
        self.add_parameter(name = 'current_ch'+str(channel),
                           label = 'Current_ch'+str(channel),
                           unit= 'A',
                           get_cmd = 'ISET'+str(channel)+'?',
                           set_cmd = 'ISET'+str(channel)+':{}',)
        self.add_parameter(name = 'voltage_ch'+str(channel),
        ...
        self.add_parameter(name='get_output_current_ch' + str(channel),
                            label='Output_current_ch' + str(channel),
                            unit='A',
                            get_cmd='IOUT' + str(channel) + '?',)
        self.add_parameter(name='get_output_voltage_ch' + str(channel),
