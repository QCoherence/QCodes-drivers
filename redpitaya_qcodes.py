...
class GeneratedSetPoints(Parameter):
    ...
    def get_raw(self):
        return np.linspace(
            self._startparam(), self._stopparam() - 1, self._numpointsparam()
        )
...
class Redpitaya(VisaInstrument):
    ...
    def __init__(self, name, address, **kwargs):
        ...
        self.add_parameter(
            ...
            vals=vals.Numbers(
                0, 8191 * 8e-9
            ),  # 8192 is the maximum number of samples that can be generated (65 us)
        ...
        self.add_parameter("data_size", get_cmd="OUTPUT:DATASIZE?")
        ...
        self.add_parameter(
            "length_time",  # total number of data points
            set_cmd="{}",
            get_parser=int,
        )
        # initial_value = int(0))
        ...
        )  # Maybe change the name to something understandable
        ...
        self.add_parameter(
            ...
            vals=Arrays(
                shape=(tuple([self.nb_measure.get_latest, self.dummy_array_size_4]),)
            ),
        )
        ...
        self.add_parameter("RESET", get_cmd=self.reset)
    ...
    def fill_LUT(self, function, parameters):
        ...
        if function == "SIN":
            ...
            if (
                freq > 1.0 / 8e-9
                or Amplitude > 1
                or pulse_duration + delay > 8e-9 * 8192
            ):
                ...
                Amp_bit = (
                    Amplitude * 8192
                )  ######### the DAC is 14 bit 8192 . The maximum aplitude will be (2^14)/2
                t = np.linspace(0, 2 * np.pi, N_point)
                return Amp_bit * np.concatenate(
                    (np.zeros(int(round(delay / 8e-9))), np.sin(n_oscillation * t))
                )
            ...
            if (
                ...
                t = np.linspace(0, 2 * np.pi, N_point)
                return Amp_bit * np.concatenate(
                    (np.zeros(int(round(delay / 8e-9))), np.cos(n_oscillation * t))
                )
            ...
            if (
                freq > 1.0 / 8e-9
                or Amplitude > 1
                or 2 * pulse_duration + delay + t_wait > 8e-9 * 8192
            ):
                raise ValueError(
                    "One of the parameters is not correct is the Ramsey LUT"
                )
            else:
                N_point = int(round(pulse_duration / 8e-9))
                n_oscillation = freq * pulse_duration
                ...
                return Amp_bit * np.concatenate(
                    (delay_vec, excitation_vec, wait_vec, excitation_vec)
                )
            ...
            if (
                freq > 1.0 / 8e-9
                or Amplitude > 1
                or 4 * pulse_pi_2 + delay + 2 * t_wait > 8e-9 * 8192
            ):
                ...
                return Amp_bit * np.concatenate(
                    (delay_vec, pi_2_vec, wait_vec, pi_vec, wait_vec, pi_2_vec)
                )
            ...
            return Amp_bit * np.concatenate(
                (delay_vec, slope_vec, pulse_vec, slope_vec[::-1])
            )
    ...
    def get_data(self):
        ...
        N_single_trace = int(round(self.stop_ADC() / 8e-9)) - int(
            round(self.start_ADC() / 8e-9)
        )
        # print(1,t)
        ...
        while t < nb_measure:
            try:
                ...
                if rep[1] == "}":
                    ...
                    rep = eval(
                        "[" + rep[3:-1] + "]"
                    )  ### it creates an array of the data removing just the first one
                    signal = np.concatenate((signal, rep))
                    tick = np.bitwise_and(
                        rep, 3
                    )  # extraction du debut de l'aquisition: LSB = 3
        ...
        if mode == "ADC" or mode == "IQCH1" or mode == "IQCH2":
            # print(12)
            data_1 = signal[::2] / (4 * 8192.0)
            data_2 = signal[1::2] / (4 * 8192.0)
            return data_1, data_2
        else:  # mode IQint and IQLP1
            ICH1 = (
                signal[::4] / (4 * 8192.0) / N_single_trace
            )  # (self.length_time()/self.nb_measure()) # it staRT FROM 0 AND TAKE A POINT EVERY 4
            QCH1 = (
                signal[1::4] / (4 * 8192.0) / N_single_trace
            )  # (self.length_time()/self.nb_measure()) # it staART FROM 1 AND TAKE A POINT EVERY 4
            ICH2 = (
                signal[2::4] / (4 * 8192.0) / N_single_trace
            )  # (self.length_time()/self.nb_measure())
            QCH2 = (
                signal[3::4] / (4 * 8192.0) / N_single_trace
            )  # (self.length_time()/self.nb_measure())
    ...
    def get_single_pulse(self):
        ...
        if self.mode_output() == ("ADC" or "IQCH1" or "IQCH2"):
            ...
            ICH1 = signal[::4] / (4 * 8192.0)
            QCH1 = signal[1::4] / (4 * 8192.0)
            ICH2 = signal[2::4] / (4 * 8192.0)
            QCH2 = signal[3::4] / (4 * 8192.0)
            return ICH1, QCH1, ICH2, QCH2
    ...
    def get_data_binary(self, mode, nb_measure):
        ...
        if mode == ("ADC" or "IQCH1" or "IQCH2"):
            data_1 = signal[::2][:nb_measure] / (4 * 8192.0)
            data_2 = signal[1::2][:nb_measure] / (4 * 8192.0)
            return data_1, data_2

        else:
            ICH1 = signal[::4][:nb_measure] / (4 * 8192.0)
            QCH1 = signal[1::4][:nb_measure] / (4 * 8192.0)
            ICH2 = signal[2::4][:nb_measure] / (4 * 8192.0)
            QCH2 = signal[3::4][:nb_measure] / (4 * 8192.0)
            return ICH1, QCH1, ICH2, QCH2
