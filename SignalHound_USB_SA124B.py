...
class SignalHound_USB_SA124B(Instrument):
    ...
    def mod_rbd_process(self):
        if self.mod_rbw_mult() != 1:
            self.span(
                int(self.span() / self.mod_rbw_mult() + 1e6) * self.mod_rbw_mult() - 1e6
            )
    ...
    def _get_sweep_data(self) -> np.ndarray:
        ...
        if self.mod_rbw_mult() != 1:
            data = np.mean(
                data.reshape(int(sweep_len / self.mod_rbw_mult()), self.mod_rbw_mult()),
                axis=1,
            )
    ...
    def _get_freq_axis(self) -> np.ndarray:
        ...
        if self.mod_rbw_mult() != 1:
            freq_points = np.mean(
                freq_points.reshape(
                    int(sweep_len / self.mod_rbw_mult()), self.mod_rbw_mult()
                ),
                axis=1,
            )
