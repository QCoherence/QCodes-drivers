...
class SignalHound_USB_SA124B(Instrument):
    ...
    def __init__(self, name, dll_path=None, **kwargs):
        ...
        self.add_parameter(
            "running", get_cmd=None, set_cmd=None, initial_value=False, vals=vals.Bool()
        )
        self.add_parameter(
            "decimation",
            get_cmd=None,
    ...
    def openDevice(self):
        ...
        if ret != self.saStatus["saNoError"]:
            if ret == self.saStatus["saNullPtrErr"]:
                ...
                raise ValueError(
                    "Could not open device due to unknown reason! Error = %d" % ret
                )
    ...
    def preset(self):
        ...
        self.log.warning(
            "Please ensure you close the device handle within two seconds of this call!"
        )
    ...
    def initialisation(self, flag=0):
        ...
        if mode in modeOpts:
            ...
            raise ValueError(
                "Mode must be one of %s. Passed value was %s." % (modeOpts, mode)
            )
    ...
    def configure(self, rejection=True):
        ...
        if self.acquisition_mode() in detectorVals:
            ...
            raise ValueError(
                "Invalid Detector mode! Detector  must be one of "
                "%s. Specified detector = %s" % (list(detectorVals.keys()), detector)
            )
        if self.scale() in scaleVals:
            scale = scaleVals[self.scale()]
        else:
            raise ValueError(
                "Invalid Scaling mode! Scaling mode must be one "
                "of %s. Specified scale = %s" % (list(scaleVals.keys()), scale)
            )
        ...
        if self.device_mode() == "sweeping":
            ...
            err = self.dll.saConfigSweepCoupling(
                self.deviceHandle,
                ct.c_double(self.get("rbw")),
                ct.c_double(self.get("vbw")),
                reject_var,
            )
            self.check_for_error(err)
    ...
    def sweep(self):
        ...
        sleep(0.1)  # Added extra sleep for updating issue
        err = self.dll.saGetSweep_32f(self.deviceHandle, minarr, maxarr)
        sleep(0.1)  # Added extra sleep
