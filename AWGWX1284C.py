...
class Tabor_WX1284C(VisaInstrument):
    ...
    def __init__(self, name, address, timeout=180, num_channels=4, **kwargs):
        ...
        for i in range(1, self.num_channels + 1):
            ...
            self.add_parameter(
                "ch{}_freq_std".format(i),
                label="Frequency std channel {}".format(i),
                unit="Hz",
                get_cmd=freq_cmd + "?",
                set_cmd=freq_cmd + " {:.6f}",
                vals=vals.Numbers(10.0e3, 1.0e9),
                get_parser=float,
            )
    ...
    def set_run_mode(self, value="TRIG"):
        ...
        if value.upper() == "CONT":
            self.visa_handle.write(
                "INIT:CONT ON"
            )  # _visainstrument.write('INIT:CONT ON')
        ...
        #         logging.info('Run mode wasn\'t set properly')
            ...
            print(
                "The invalid value {} was sent to set_run_mode method. Valid values are 'CONT', 'TRIG', 'GATE'.".format(
                    value
                )
            )
