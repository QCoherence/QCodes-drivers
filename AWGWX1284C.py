...
Mark_num = (1,2)
###### Useful functions
def engineer_to_scienc(value_with_newline):
        '''
        ...
        Output:
            converted value (int)
        '''
        value = value_with_newline.strip().strip('\n') # Strip the new line
...
class Tabor_WX1284C(VisaInstrument):
    """
        This is the Qcodes driver for the Tabor WX1284C
    ...
    def __init__(self, name, address, timeout=180, num_channels=4, **kwargs):
            ...
            self.add_parameter('run_mode',
                           ...
                           #,
                           # vals=vals.Enum('CONT', 'TRIG',  'GATE'),
                           # get_parser=self.newlinestripper
                           # )

            self.add_parameter('clock_source',
                           label='Clock source',
                           get_cmd=':FREQ:RAST:SOUR?',
                           set_cmd=':FREQ:RAST:SOUR ' + '{}',
            ...
            self.add_parameter('trigger_source',
                           ...
                           vals=vals.Enum('EXT', 'BUS', 'TIM', 'EVEN'),
                           get_parser=self.newlinestripper)
            ...
            self.add_parameter('trigger_mode',
                           get_cmd=':TRIG:MODE?',
                           set_cmd=':TRIG:MODE ' + '{}',
                           vals=vals.Enum('NORM', 'OVER'),
                           get_parser=self.newlinestripper)


            self.add_parameter('trigger_timer_mode',
                           get_cmd=':TRIG:TIM:MODE?',
                           set_cmd=':TRIG:TIM:MODE ' + '{}',
                           vals=vals.Enum('TIME', 'DEL'),
                           get_parser=self.newlinestripper)
...
##### NEED TO CHECK THE SET PART
            self.add_parameter('ref_freq',
                           label='Reference frequency',
                           unit='Hz',
                           get_cmd=':ROSC:FREQ?',
                           set_cmd=':ROSC:FREQ ' + '{}',
                           vals=vals.Enum(10e6, 20e6, 50e6, 100e6),
                           get_parser=engineer_to_scienc)


            self.add_parameter('channels_synchronised',
                            label = 'Channels Synchronised',
                            get_cmd = 'INST:COUP:STAT?',
                            set_cmd = 'INST:COUP:STAT' + '{}',
                            vals=vals.Enum('ON', 'OFF'),
                            get_parser=self.newlinestripper)

            # channels_synchronised



        # Channel parameters #
            for i in range(1, self.num_channels+1):
                amp_cmd = 'INST:SEL{};VOLT:AMPL'.format(i)
                offset_cmd = 'INST:SEL{};VOLT:OFFS'.format(i)
                state_cmd = 'INST:SEL{};OUTP'.format(i)
                waveform_cmd = 'INST:SEL{};:FUNC:SHAP'.format(i)
                coupling_cmd = 'INST:SEL{};OUTP:COUP'.format(i)
                freq_cmd = 'INST:SEL{};FREQ:CW'.format(i)
            ...
            # # Set channel first to ensure sensible sorting of pars
                ...
                self.add_parameter('ch{}_waveform'.format(i),
                                   ...
                                   vals=vals.Strings(),
                                   get_parser=parsestr)
                self.add_parameter('ch{}_coupling'.format(i),
                                   label='Coupling channel {}'.format(i),
                                   get_cmd=coupling_cmd + '?',
                                   set_cmd=coupling_cmd + ' {}',
                                   vals=vals.Enum('DC', 'HV'),
                                   get_parser=parsestr)
                self.add_parameter('ch{}_freq_std'.format(i),
                                   label='Frequency std channel {}'.format(i),
                                   unit='Hz',
                                   get_cmd=freq_cmd + '?',
                                   set_cmd=freq_cmd + ' {:.6f}',
                                   vals=vals.Numbers(10.e3, 1.e9),
                                   get_parser=float)
...
# Functions
    ...
    def set_run_mode(self, value='TRIG'):
        '''
        ...
        if value.upper() == 'CONT':
            self.visa_handle.write('INIT:CONT ON') # _visainstrument.write('INIT:CONT ON')
            ...
            self.visa_handle.write('INIT:GATE:STAT OFF')
            ...
            self.visa_handle.write('INIT:CONT:STAT OFF')
            self.visa_handle.write('INIT:GATE:STAT ON')
            # if self._visainstrument.query('INIT:CONT?') != 'OFF':
            #     logging.info('Run mode wasn\'t set properly')
        ...
        #         logging.info('Run mode wasn\'t set properly')
            ...
            print('The invalid value {} was sent to set_run_mode method. Valid values are \'CONT\', \'TRIG\', \'GATE\'.'.format(value))
