            ...
            for n in range(self.N_ramp):
                if self.half_period < max_adwin_ramp_time:
                    ...
                    self.instrument.subsampling(
                        2
                        * int(
                            ((subdivision + 1) * self.instrument.ramp_size())
                            / self.n_pts
                        )
                    )
...
class ADwin_averagedRamp(MultiParameter):
    def __init__(
        ...
        **kwargs,
        ...
        input_number = instrument.get_input_number()
        doubled_input_number_list = sorted(
            2 * input_number
        )  # double the occurence of each input number to label the up/down part of the ramp
        super().__init__(
            name,
            instrument=instrument,
            names=tuple(
            ...
            setpoint_names=tuple(
                (f"input_{e + 1}_{'down' if i % 2 else 'up'}_points",)
                for i, e in enumerate(doubled_input_number_list)
            ),
        )
        self.period = period
...
class ADwin_IV(MultiParameter):
    def __init__(
        ...
        if len(avgRamp.instrument.get_input_number()) != 2:
            raise Exception(
                f"The Adwin should be measuring exactly two inputs for the IV (found {len(avgRamp.instrument.get_input_number())} inputs)"
            )
        ...
        super().__init__(
            ...
            metadata={
                "Voltage divider value": "%.2e" % voltage_divider_value,
                "Current coefficient": "%.2e" % current_coefficient,
            },
        )
...
class ADwin_VI(MultiParameter):
    ...
    def __init__(
        ...
        if len(avgRamp.instrument.get_input_number()) != 2:
            raise Exception(
                f"The Adwin should be measuring exactly two inputs for the VI (found {len(avgRamp.instrument.get_input_number())} inputs)"
            )
        ...
        super().__init__(
            ...
            metadata={
                "Current divider value": "%.2e" % current_divider_value,
                "Voltage coefficient": "%.2e" % voltage_coefficient,
            },
        )
...
class ADwin_Gold2(Instrument):
    ...
    def process_load(self):
        ...
        self.device.Set_Processdelay(
            1, 3000
        )  # Setting the process frequency to 100 kHz since the clock of the T11 is 3.33 ns
    ...
    def get_input_size(self):
        inputs_number = bin(
            ...
            "1"
        )  # count the number of 1 in input_mask by converting it to string and using .count (horrible but it works)
